...
async function main() {
    console.log("🚀 Starting Fresh Polygon Mainnet Deployment...\n");
    ...
    try {
        // Force new deployment by using different options
        const deployOptions = { 
        ...
        const claimTopicsAddr = await claimTopicsRegistry.getAddress();
        deployment.contracts.ClaimTopicsRegistry = claimTopicsAddr;
        console.log("✅ ClaimTopicsRegistry deployed to:", claimTopicsAddr);
        
        // Wait for confirmation
        console.log("   Waiting for confirmations...");
        await claimTopicsRegistry.deploymentTransaction().wait(5);
        ...
        deployment.contracts.IdentityRegistry = identityAddr;
        ...
        const setClaimTx = await identityRegistry.setClaimTopicsRegistry(claimTopicsAddr);
        await setClaimTx.wait(5);
        console.log("   ✅ ClaimTopicsRegistry set");
        ...
        const countryRestrictModule = await upgrades.deployProxy(CountryRestrictModule, [], deployOptions);
        ...
        const maxBalanceModule = await upgrades.deployProxy(MaxBalanceModule, [], deployOptions);
        ...
        const transferLimitModule = await upgrades.deployProxy(TransferLimitModule, [], deployOptions);
        ...
        const modularCompliance = await upgrades.deployProxy(ModularCompliance, [], deployOptions);
